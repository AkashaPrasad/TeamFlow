...
export function ProjectCard({ project, onDelete }) {
  ...
  return (
    <Link to={`/projects/${project.id}`} className="block group">
      <GlowingCard className="p-5 hover:-translate-y-0.5 transition-transform duration-200">
        {/* Header */}
        ...
        <div className="mb-3">
          <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 mb-1.5">
            ...
            <span className="font-medium text-brand-600 dark:text-brand-400">{project.progress ?? 0}%</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex -space-x-1.5">
              {members.slice(0, 4).map((m) => (
                <Avatar key={m.id} name={m.profiles?.name} src={m.profiles?.avatar_url} size="xs" className="ring-2 ring-white dark:ring-gray-800" />
              ))}
              {members.length > 4 && (
                <div className="w-6 h-6 rounded-full bg-gray-200 dark:bg-gray-700 ring-2 ring-white dark:ring-gray-800 flex items-center justify-center text-[10px] text-gray-500 dark:text-gray-400 font-medium">
                  +{members.length - 4}
                </div>
              )}
            </div>
